...
const currentScript = document.currentScript || document.querySelector('script[type="module"][src*="/plugin/dependency-track/js/result-summary-violations.js"][data-action-url]');
    ...
    const container = document.getElementById('dependency-track-findings-summary-violations-chart');
    ...
    chart.setOption({
        ...
        yAxis: {
            type: 'category',
            data: ['Fail', 'Warn', 'Info', 'Unassigned'],
            inverse: true,
            nameGap: 30,
            axisLabel: {
                interval: 0,
    ...
    .then(i18n => {
        chart.setOption({
            series: [
                { id: 'Violations', name: i18n['tooltip.title.violations'] }
            ],
            yAxis: {
                data: [i18n['seriesTitle.fail'], i18n['seriesTitle.warn'], i18n['seriesTitle.info'], i18n['seriesTitle.unassigned']],
            }
        });
    });

    window.fetch(`${actionUrl.href}/getPolicyViolationsJson`, {
        method: 'POST',
        mode: 'same-origin',
        credentials: 'same-origin',
        cache: 'default',
    ...
    .then(data => {
        if (data.length) {
            const chartData = [
                ...
                { name: 'WARN', value: 0, color: '#fd8c00' },
                { name: 'INFO', value: 0, color: '#357abd' },
                { name: 'UNASSIGNED', value: 0, color: '#c0c0c0' },
            ];
            data.forEach(violations => {
                chartData.find(item => item.name === violations.policyCondition.policy.violationState).value++;
            });
            chart.setOption({
                series: [
                    {
